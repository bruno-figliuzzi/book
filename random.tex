...
We assume now that $E$ is the euclidean space $\mathbb{R}^d$ of dimension $d$. The vectorial space structure of $\mathbb{R}^d$ allows us to define new operations on $\mathcal{P}(\mathbb{R}^d)$, namely the Minkowski addition and subtraction. 
\begin{definition}
...
\end{equation}
\begin{definition}
Using these notations, we can define the Minkowski subtraction by duality. Let $A$ and $B$ be subsets of $E$. The Minkowski subtraction is defined by
\begin{equation}
A \ominus B = (A^c \oplus B)^c = \cap_{x \in B}A_x.
\end{equation}
\end{definition}
We can also express the classical dilation and erosion operators of mathematical morphology as functions of the Minkowski addition and subtraction respectively. 
\begin{definition}
...
\begin{figure}[p]
    ...
    \begin{subfigure}[b]{0.6\textwidth}
        ...
        \caption{Segmented microstructure simulated with VtkSim~\citep{vtksim}.}
    \end{subfigure}
...
\end{itemize}
\end{definition}
The axiomatic of granulometries was first formulated in~\citep{matheron}. Note that an immediate consequence of the last point of the definition is that $\Phi_{\lambda}$ is necessarily an idempotent operator, in the sense that $\Phi _{\lambda } \circ \Phi _{\lambda } = \Phi _{\lambda }$.
...
\section{Probabilistic approach and Choquet capacity\label{models:s2}}
...
\end{equation}
$T(K)$ is called the Choquet capacity of the random closed set $A$. Note that the Choquet capacity is closely related to dilation and erosion operators. For all compact set $K \subset \mathbb{R}^d$, we have indeed
\begin{equation}
...
$$
B \ominus \check{K} := \{ x \in \mathbb{R}^d : x + \check{K} \subset B \}.
$$
Let us apply this definition with $B = A^c$. Then, for any $x \in \mathbb{R}^d$,
$$
...
\begin{definition}The covariance of a random set $A \subset \mathbb{R}^d$ is the function $C_A$ defined on $\mathbb{R}^d \times \mathbb{R}^d$ by
\begin{equation}
C_A(x, x + h) = P\{x \in A, x + h \in A \},
\end{equation}
where $h$ is some vector of $\mathbb{R}^d$.
...
\end{proposition}
\begin{proof}
Let $A \subset \mathbb{R}^d$ be a random closed set. Define its indicator function
$$
f(x) := \mathbf{1}_A(x) = 
\begin{cases}
1, & x \in A,\\
...
$$
\mathcal{F}[f * g] = \hat{f} \, \hat{g}, \quad \text{and} \quad f * g(x) = \int_{\mathbb{R}^d} f(y) g(x-y) \, dy.
$$
The covariance can be expressed as a convolution:
$$
C_A(h) = \int_{\mathbb{R}^d} f(x) f(x+h) \, dx = (f * f^\dagger)(h),
$$
...
$$
C_A(h) = \frac{1}{(2\pi)^n} \int_{\mathbb{R}^d} |\hat{f}(\xi)|^2 e^{i \xi \cdot h} \, d\xi.
$$
...
$$
corresponds to the volume of the ``boundary layer`` of thickness $\Delta h$ where $A$ and $A_{h\mathbf{u}}$ stop overlapping. By classical results in geometric measure theory (Steiner formula and Crofton formula, see section~\ref{models:s4}), this volume is approximately
$$
...
\section{Measurements on random sets \label{models:s4}}
...
Let us first introduce a general framework for the study of point processes on locally compact topological spaces. The results of this section are technical and we will only state the most relevant ones, often without proof. We refer the reader to the reference textbooks~\citep{schneider} and~\citep{stoyan} for a more extensive presentation.
...
It can be shown that $N(E)$ is a measurable subset of $(M(E), \mathcal{M})$~\citep{schneider}. We denote by $\mathcal{N}$ the corresponding $\sigma$-algebra. A fundamental example of counting measure is given by locally finite sums of Dirac measures:
\begin{equation}
...
\begin{definition} Let $\theta $ be a locally random finite measure on $\mathbb{R}^d$. A Cox-Poisson point process on $\mathbb{R}^d$ is a point process such that the number $N(K)$ of points contained in any compact $K$ of $\mathbb{R}^d$ is a Poisson random variable with parameter $\theta (K)$:
\begin{equation}
P\{N(K) = k \} = p_k(K) = \dfrac{\theta (K)^k}{k!}\exp (-\theta (K)),
\end{equation}
...
A fundamental example of Cox point processes are Poisson point processes restricted to some random closed set. Let $A$ be a random closed set of $\mathbb{R}^d$, an $\lambda > 0$. The measure
\begin{equation}
\theta(K) = \int_K \theta 1_{A}(x) dx ,
\end{equation}
where $1_A$ is the indicative function of the set $A$ defines a Cox-Poisson point process. This Cox-Poisson point process can be seen as the restriction of a stationary Poisson point process of intensity $\theta $ to the random closed set $A$. Such point processes are often used in stochastic geometry to construct multiscale models. We refer the reader interested by these models to the book~\citep{jeulin}.
...
It can be shown (see for instance~\citep{schneider}) that the intensity of a marked point process can be decomposed in the following manner
\begin{equation}
...
\begin{lemma}
Let $K$ be a compact set of $\mathbb{R}^d$ and $A$ a Boolean model with primary grain $A'$ and with intensity $\theta$. The number $N(K)$ of primary grains hit by $K$ follows a Poisson distribution of parameter $\mathbb{E}\{\theta(\check{A}\oplus K) \}$:
\begin{equation}
...
\caption{3D Realization of a Boolean model of spheres with constant radius $R$ and intensity $\theta $, realized with the software vtkSim~\citep{vtksim}. The parameters of the model are $\theta = 5 \times 10 ^{-2}$ and $R = 1 $. }
\end{figure}
...
The method of densities (or intensities) was developed by Weil in 1984~\citep{weil1984} and Santal\'o~\citep{santalo} to recover the parameters of a Boolean from a given dataset. The main idea of the method is stated in proposition~\ref{prop:densities} below.
...
For models with convex grains, it is possible to relate the mean values of the intrinsic volumes of the typical grain to measurements conducted on the global dataset through Miles' formulae~\citep{miles1976}. For $d = 2$, Miles' formulae yield
\begin{eqnarray}
A_A  & = & p = 1 - \exp(-\theta \bar{A}), \\
L_A  & = &\theta (1 - p) \bar{S} = \theta \exp(-\theta \bar{V})\bar{S},\\
N_A & = & \theta(1 - p) \bigg(1 - \dfrac{\theta \bar{L}^2}{4 \pi} \bigg).
\end{eqnarray}
For $d = 3$, Miles' formulae yield
\begin{eqnarray}
V_V  & = & p = 1 - \exp(-\theta \bar{V}), \\
S_V  & = &\theta (1 - p) \bar{S} = \theta \exp(-\theta \bar{V})\bar{S},\\
...
\end{eqnarray}
Hence, when estimates of the densities are given, the intensities of the Boolean model can easily be found. We refer the reader interested by a proof of Miles' formulae to the original paper~\citep{miles1976} and to the book~\citep{schneider}. \\
...
The Boolean model is an archetypal model of stochastic geometry. Reference textbooks on this topic include~\citep{matheron},~\citep{serra1982},~\citep{stoyan} and~\citep{baccelli}. We also refer the reader to the lecture notes~\citep{jeulin}. For an extensive presentation, we refer the reader to the book~\citep{schneider}.
 
Regarding statistical analysis for the Boolean model, we refer the reader to the papers\citep{weil1984} and~\citep{molchanov1995}. Miles' formulae were derived by Miles in 1976~\citep{miles1976}. An introduction can be found in the book~\citep{stoyan}. This topic is discussed more thoroughly in the book~\citep{schneider}.

Another topic of interest regarding the Boolean model is percolation. This topic has been studied extensively over the years. We refer the reader interested by this the percolation properties of the Boolean model to the papers~\citep{hall, moreaud, figliuzzi2016}, and to the book~\citep{torquato}.

In materials engineering, the Boolean model has been employed to simulate a wide range of microstructures. Various examples of the application of the Boolean model in materials science are described in the paper~\citep{hermann}. In 1992, Quenec'h \textit{et al.} used the Boolean model to study the growth of WC grains in WC-Co cermets~\citep{quenec}. In 2001, Jeulin et \textit{al.} relied on Poisson polyhedra to simulate the microsctructure of needle-shaped gypsum crystal grains~\citep{jeulin1}. In 2003, Capasso studied the application of the Boolean model to the description of crystallisation in metals and polymers. More recently, Jean et \textit{al.} simulated the microstructure of rubbers by considering a multiscale Cox-Boolean model~\citep{jean}. Using a random walk based model, Altendorf and Jeulin developed a stochastic model for simulating 3D fiber structures~\citep{altendorf}. Pereyga \textit{et al.} relied on a Boolean model of random cylinders to model a random fibrous network~\citep{peyrega}.

\section{Random Tessellations}

A \textit{tessellation} or \textit{mosaic} is a division of the $d-$dimensional Euclidean space $\mathbb{R}^d$ into polyhedra. Such geometrical patterns can be observed in many natural situations. Hence, random tessellation models have been widely used in physics, materials science and chemistry.

\subsection{General introduction \label{tesselation:ss1}}
...
\end{equation}
\end{proposition}
These relation are derived by considering the topological configuration of random mosaics. Some results are particularly obvious. For instance, it is clear that the number of neighbor vertices for a given edge is $n_{10} = 2$. Note that similar relations can be obtained in higher dimensions. We refer the reader to the book~\citep{schneider} for a more extensive presentation of the theory and to the book~\citep{stoyan} for the case $d = 3$. 
...
\caption{Vorono\"i tessellation in $\mathbb{R}^3$. The simulation has been obtained with the software VtkSim~\citep{vtksim}. The center of the Vorono\"i cells are represented in grey.}
\end{center}
\end{figure}
...
\caption{Vorono\"i tessellation in $\mathbb{R}^3$. The simulation has been obtained with the software VtkSim~\citep{vtksim}. The center of the tessellation cells are represented in grey. The first germs are represented with larger radii.  }
\end{center}
\end{figure}
...
Random tessellations constitute an active topic in stochastic geometry. Reference textbooks on this topic include~\citep{matheron},~\citep{stoyan}. For an extensive presentation, we refer the reader to the book~\citep{schneider}, where a proof is given for almost all results. The mean-value relationships given in section~\ref{tesselation:ss1} are mostly due to the studies~\citep{mecke1980} and~\citep{moller1989}.

Poisson hyperplanes tessellations and Poisson polyhedra have been extensively studied in~\citep{matheron}, ~\citep{serra1982} and Miles~\citep{miles1972b}. We also refer the reader to the paper~\citep{mecke1995}.

The Vorono\"i tessellation is a classical model in stochastic geometry. A general description of Poisson-Vorono\"i tessellations in $\mathbb{R}^d$ can be found in~\citep{moller1989, moller1994}. The Johnson-Mehl tessellation model was introduced by Johnson and Mehl to describe crystallization processes~\citep{johnson, avrami, gilbert}. Their model can be seen as a variation of the Vorono\"i model. The paper~\citep{moller1992} provides a unified exposition of Random Johnson-Mehl tessellations.
